...
const int screenHeight = 3000;

const int CenterX = 1500;
const int CenterY = 1500;
const Vector2 Center = {1500, 1500};

const int BirdNum = 3250;
...
// alignement
const int AlignRange = 100;
const float AlignScale = 0.2;
// cohesion
const int CoheRange = 1000;
const float CoheScale = 0.000075;
const int CoheSepDelta = 100;
...
inline short CalculatePart(struct Birds b) {
    // seprate into 4 parts
    if (b.position.x - CenterX < 0) {
        if (b.position.y - CenterY < 0) {
            return 1;
        } else {
            return 2;
        }
...
struct Birds UpdateBird(struct Birds bird, struct Birds array[BirdNum], Vector2 MousePos) {
    ...
    for (int i = 0; i < BirdNum; i++) {
        if (bird.Part == array[i].Part) {// spatial partitionign
...
void DrawPartLines() {
    Vector2 lxcy = {0, CenterY};
    Vector2 rxcy = {screenWidth, CenterY};
    Vector2 cxuy = {CenterX, 0};
    Vector2 cxdy = {CenterX, screenHeight};
    DrawLineV(lxcy, rxcy, BLUE);
    DrawLineV(cxuy, cxdy, BLUE);
}
...
int main() {
    ...
    while(!WindowShouldClose()){
        ...
        for (int i = 0; i < BirdNum; i++) {
            BirdArray[i].Part = CalculatePart(BirdArray[i]);
        }
        for (int i = 0; i < BirdNum; i++) {
